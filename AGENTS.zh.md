...
项目文档同时维护英文与中文。默认 .md 文件使用英语，同目录的 .zh.md 为中文版本，
包括 README、AGENTS、CHANGELOG、架构说明、指南和 ADR。内容变化时同步两版并保持语言互链，
站内链接优先指向同语言版本。示例的命令、协议字段和用户原文保留原义；许可证原文与测试夹具不翻译。
新的提交信息使用英语，已有提交历史保持不变。Web UI 支持英语和简体中文，界面文案在同一词典中维护。
...
同一主题每种语言各维护一份，其他文档链接过去。
README、AGENTS 和 CHANGELOG 的两种语言版本留在根目录，供用户和 Agent 进入项目。其余文档放在 docs，
操作指南归入 guides，决策记录归入 adr；架构说明与后续计划各保留一份。

| 文档 | 读者与职责 |
| --- | --- |
| README.md / README.zh.md | 向用户介绍产品用途、安装与连接，并说明权限和使用限制。 |
| [配置指南](docs/guides/configuration.zh.md)、[连接指南](docs/guides/connections.zh.md)及[用法示例](docs/guides/code-mode-examples.zh.md) | 供用户和调用者按需查阅操作方法。 |
| AGENTS.md / AGENTS.zh.md | 供 Agent 查阅开发规则和阅读入口。 |
| [更新日志](CHANGELOG.zh.md) | 记录已发布版本的用户可见变化，两版同步；当前只保留 1.0.0。 |
| [架构说明](docs/architecture.zh.md) | 向开发者与 Agent 说明长期职责划分。函数和文件关系直接读代码。 |
| docs/adr/ | 主要供 Agent 查阅当前生效的选择、理由和代价。 |
| [后续计划](docs/backlog.zh.md) | 记录用户明确保留、尚未交付的计划与前置条件。 |
...
| 任务 | 生效决定 |
| --- | --- |
| 工具入口、工作目录、补丁、Codex 复用、跨平台 | [ADR-001 执行内核与产品边界](docs/adr/001-exec-runtime.zh.md) |
| ALL_TOOLS、契约发现、英文模型描述、下游 MCP | [ADR-002 工具发现与契约](docs/adr/002-tool-discovery.zh.md) |
| JSON 去重、显式预算、store/load、媒体、cell/进程 | [ADR-003 结果与执行生命周期](docs/adr/003-results-lifecycle.zh.md) |
| Tunnel、认证、安装、发布及机器权限 | [ADR-004 接入与信任边界](docs/adr/004-connectivity-trust.zh.md) |
| 原生附件、文件导入导出、资源读取与下载 URL | [ADR-005 文件传输与交付边界](docs/adr/005-file-transfer.zh.md) |
| Skill 发现、软链接、显式调用策略与目录预算 | [ADR-006 一次发现的 Skill 目录](docs/adr/006-skill-catalog.zh.md) |
| Shell 默认值、单次覆盖及精简动态契约 | [ADR-007 默认命令 Shell 与单次覆盖](docs/adr/007-command-shell.zh.md) |
| 完整环境继承、出站代理与 Node 20 兼容 | [ADR-008 环境、代理与 Node 20](docs/adr/008-environment-and-proxy.zh.md) |
| Web 控制台、局域网认证、审计边界与前端交付 | [ADR-009 可选的本机 Web 管理控制台](docs/adr/009-web-console.zh.md) |
| 会话备注、异步提问、Web 作答与 User Note FIFO | [ADR-010 Web 会话补充与异步提问](docs/adr/010-session-notes.zh.md) |
