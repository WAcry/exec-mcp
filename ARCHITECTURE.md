...
这是长期职责划分，不是函数或文件地图。用户入口见 README，操作设置见 docs/configuration.md 和 docs/connections.md。
...
每个连接指向一台具体远端机器；文中的本机指服务所在机器，不是 ChatGPT 的其他容器。
V8 的无文件/网络 API 只描述编排 JavaScript，工具仍访问该机器实际的文件系统与网络。
...
私有 OpenAI Tunnel 与已支持的 Cloudflare/Tailscale 公网接入使用不同认证边界，见
[ADR-004](docs/adr-004-connectivity-trust.md)。

目录结构、函数关系、具体阈值和协议字段应直接读代码与测试，
不继续扩写这份文档来镜像实现。
