...
启用 Web UI 并通过 CLI 启动后，ChatGPT 可以在 exec 中调用 `tools.request_user_input_async`：

```js
text(await tools.request_user_input_async({
  request_key: "storage-choice-v1",
  questions: [{ title: "新模块使用哪种存储？", options: ["SQLite", "PostgreSQL"] }]
}));
// 已保存，继续做不依赖答案的工作。
```

沿用 Codex 的 title/options 形状；同一对话内 request_key 去重，同键不同问题会报冲突。
选项按展示顺序给出，首项标记推荐但不预选；Web 支持选择＋补充说明、以上都不是＋自定义回答，以及无选项的文本问答。
每题可分别提交或修改；并发修改有版本冲突提示，不静默覆盖你的输入。

问题属于原 ChatGPT 对话，需要宿主提供 `openai/session`，不绑定 cell、终端或 MCP 网络连接。
用户答复在之后 exec/wait 的正常响应中附加；Agent 在下一次调用的 `ack_user_input` 数组中确认收到的 event_id。
未确认允许重复送达，修改答案产生新事件；状态区分“已保存”“已尝试投递”“Agent 已确认接收”，确认不等于执行完成。
支持 `tools.get_user_input({request_id})` 查看当前对话状态；调用均不等待用户。
依赖答案的操作必须先让答案返回模型，再继续决策，不应在收到答复前执行写死的方案。

控制台的“异步问答”页持续显示待答数量。点击“启用系统通知”并授权后，页面打开时可收到浏览器提醒；
支持 Notifications API 的浏览器可在 HTTPS 或本机回环地址申请通知权限；系统是否展示也受操作系统设置影响。
普通 HTTP 局域网地址可能只能页面提醒。关闭页面后不保证通知，
没有 ChatGPT Widget，也不主动启动 ChatGPT 的新一轮。wait 只在正常返回时搭载答案，不会因人作答而即时中断。
ChatGPT 已结束本轮时，可直接复制页面中的问题、选择和完整 notes 到原对话。

记录保存在配置文件旁的 `.exec-mcp/<配置文件名>.questions.sqlite3`（包含 SQLite 运行时的 WAL/SHM 伴随文件）。
待回答和最新未确认答复不自动过期；整组已确认历史在 30 天后可清理。host 内存回收、清空审计、重启执行服务
均不会清除问答，但不会恢复原来的脚本或进程。数据库是本机敏感数据，按运行账户权限保存，不应公开分享。
