...
store/load 是有配额的对话缓存，不是永久存储。无资源压力时，连续空闲 **72 小时**才过期，
每次使用会刷新空闲时间；达到总容量后优先回收最久未使用的空闲对话，不清理活动任务。
当前默认每对话 **32 MiB、4096 个键**，总计 **256 MiB、128 个对话**；用量包含序列化及键管理开销，
不是进程 RSS 上限。单对话超限会拒绝写入，不静默删掉该对话的数据。助手可用
`store.delete(key)`、`store.clear()` 释放数据，用 `store.stats()` 查看当前快照用量。
写入在完成结果由 exec/wait 收取时合并；需长期保留的数据应写文件，而不是只放进缓存。

最多保留 **16 个运行中、结果待取或清理中的 cell**，满额后先收取或终止已有 cell；
不会仅因等待超过半小时而终止任务。`/readyz` 的 `code_mode` 字段提供缓存用量、压力回收和 cell 数量，
不包含缓存键、值或对话标识。已有配置显式写出的文件大小和有效期不随默认值变化而被覆盖。
