...
任务默认由 ChatGPT 自行完成；只有用户明确要求，才可调用远端机器上的其他 Agent CLI（如 Codex、Claude Code）。
机器上已经安装或登录不等于获准消耗其额度。这是工具描述中的授权边界，不增加命令拦截或审批流程；
固定 Codex Code Mode host 和 patch engine 的本地执行不属于委派给另一个 Agent。

破坏性删除命令的目标须是明确、完整、已解析的绝对路径字面量，执行前核验最终目标与删除范围；
不使用 Shell 变量、命令替换、通配符、动态拼接或其他路径插值，以免实际删除范围被展开改变。
这是操作者明确要求的调用规范，只在 exec 描述中说明，不增加命令拦截、路径改写或审批机制。
