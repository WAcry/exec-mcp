...
连接器名称由宿主分配，描述使用“本连接器的 exec source”来指明 ALL_TOOLS 目录与 tools 绑定所在的环境。
外层编排环境有独立目录，也可能重新包装连接器响应；嵌套方法的返回类型只描述 source 内拿到的值。
文档说明这些边界，不写死连接器别名，也不为某个宿主的包装方式改变 MCP 返回格式。
...
本实例向宿主交付 export_file 的方向相反，见 [ADR-005](005-file-transfer.zh.md)。

## 模型契约与产品文档

本项目生成的 MCP instructions、工具及资源说明、schema 描述和 ALL_TOOLS 包装使用英语，
便于沿用 Codex 的同义原文；token 用量取决于具体模型和文本。
项目文档的默认 .md 为英文，同目录 .zh.md 为中文；配对文件同步内容并互链，维护规则见 [AGENTS](../../AGENTS.zh.md)。
Web UI 支持英语与简体中文。用户补充、Skill 与第三方契约保留原文，
运行诊断也不随浏览器语言改写。
...
完整语法和 schema 从代码生成。用户操作示例见 [Code Mode 示例](../guides/code-mode-examples.zh.md)，
配置方法见[配置指南](../guides/configuration.zh.md)，工具描述无需重复这些教程。
