...
[English](004-connectivity-trust.md) | 简体中文
...
子进程完整继承服务环境，出站代理的范围见 [ADR-008](008-environment-and-proxy.zh.md)。
...
只有显式选择该私有单操作者模式，才允许本地 MCP 省略应用认证；该模式也信任本机其他进程。
...
安装和身份提供方要求见 [连接方式](../guides/connections.zh.md)。取消与响应须按实际协议实现，
...
该适配沿用 [ADR-008](008-environment-and-proxy.zh.md) 的完整环境继承规则。
...
Web 登录密钥也使用同一保护，长期 Cookie 和轮换规则见 [ADR-009](009-web-console.zh.md)。
...
[connections.md](../guides/connections.zh.md) 供用户查阅 Auth0 设置与启动步骤，并与这里的决定保持一致。
...
本服务的文件绑定及独立下载入口见 [ADR-005](005-file-transfer.zh.md)，下载入口只能公开显式导出的文件，
不得连同无认证的执行入口一起公开。

README 写已可用的安装和连接步骤，未交付能力注明状态。
独立 Web 控制台见 [ADR-009](009-web-console.zh.md)，下游登录仍在供应商客户端完成。
下游启动检查和失败处理见 [ADR-002](002-tool-discovery.zh.md)。

## 版本与发布

1.0.0 是首个正式版本，目前通过源码构建使用。公共 npm 分发仍待确定许可、权限和发布流程。
CHANGELOG.md 与 CHANGELOG.zh.md 记录同一发布条目；新增记录不触发 tag、Release 或 npm 发布。
