...
本文说明长期职责划分。用户从 [README](../README.zh.md) 开始，开发规则见 [AGENTS.md](../AGENTS.zh.md)。
配置与连接步骤分别见[配置指南](guides/configuration.zh.md)和[连接指南](guides/connections.zh.md)。
...
生命周期及输出取舍见 [ADR-010](adr/010-session-notes.zh.md)。
Web 默认只监听回环地址，局域网认证和审计处理见 [ADR-009](adr/009-web-console.zh.md)。

Skill 发现只返回远端文档的元数据，Agent 按需读取正文或运行配套脚本。
一次返回完整目录、解析软链接和显式调用策略的取舍见 [ADR-006](adr/006-skill-catalog.zh.md)。
...
用户程序是否采用代理由自身网络栈决定，见 [ADR-008](adr/008-environment-and-proxy.zh.md)。
默认 Shell 在启动时确定，执行器与工具描述共用该值；单次覆盖只影响新命令，见 [ADR-007](adr/007-command-shell.zh.md)。
执行内核的整体取舍见 [ADR-001](adr/001-exec-runtime.zh.md)。

文件操作由 exec 编排，宿主在 exec.files 绑定输入，机器端按索引流式导入。
显式导出后，结果适配层附上原生资源链接，由独立通道传输文件快照。
文件字节直接走传输通道，省去 V8 和模型搬运 Base64 的过程，也无需修改固定 host，见 [ADR-005](adr/005-file-transfer.zh.md)。

## 目录、取消与认证

完整工具目录保存在运行时，模型只看到显式输出的条目。工具绑定、查询与展示共用一个契约来源，
下游全量 schema 按需读取，见 [ADR-002](adr/002-tool-discovery.zh.md)。

取消等待只停止本次观察，已经发生的操作仍然有效。Agent 决定展示多少结果，
服务检查传输和资源限制，并报告执行状态及无法确定的副作用，见 [ADR-003](adr/003-results-lifecycle.zh.md)。

隧道负责连接，认证负责判断谁能调用。OpenAI 私有 Tunnel 与 Cloudflare/Tailscale 公网接入
分别采用相应认证规则，见 [ADR-004](adr/004-connectivity-trust.zh.md)。
