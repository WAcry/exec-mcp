...
本文面向安装和维护 exec-mcp 的用户。首次启动见 [README](../README.md)，
Tunnel、Auth0 和 token 文件见[连接指南](connections.md)；这里的片段按需加入同一份 config.toml。

## 配置文件

`init` 创建配置并打印位置，不覆盖已有文件。默认位置：
...
`serve` 使用指定端口；冲突会报错而不是抢占其他进程。`/readyz` 可检查本机 MCP 就绪状态。
配置修改通常需要重启执行服务；工具描述改变后还需刷新 ChatGPT 的连接元数据。

## 下游 MCP

可以同时配置本机 stdio 服务和 HTTP 服务：
...
| `tool_timeout_sec` | 默认 120 秒，约束下游工具及资源请求；不是外层 wait 的等待窗口。 |
...
本服务给 ChatGPT 配置的 Auth0 认证与访问下游的凭据是两回事。
...
但是否采用由各自网络栈决定；这不是系统级 VPN，不强制代理任意浏览器、用户脚本或 UDP 流量。
...
不希望按任务自动采用某个流程时，在其 `agents/openai.yaml` 中设置：
...
此时只列名称和路径，标记“仅用户明确要求使用”，不展示触发描述。配置禁用和仅显式调用都不是文件权限隔离。
Skill 文件修改后重新发现即可；修改 exec-mcp 配置需重启。不会读取或修改 Codex 自己的启停配置。

`max_chars` 是目录的 Unicode 字符目标，不是精确 token 数；目录也会适应模型响应的字节预算。
...
Web 凭据自动保存在配置文件旁的 `.exec-mcp/<配置文件名>.web-token`，复用 token 文件的轻量加密保护，
不是浏览器明文保存的访问密钥，也不写入 config.toml。首次生成或换新时目录需可写；已保存凭据损坏时明确报错，
不会静默生成另一个密钥。保留这份私有文件即可在重启后继续登录；删除它再启动会生成新密钥并使旧登录失效。
...
计数对应当前保留记录，不是自启动以来的永久统计。裁剪不改变实际调用，配置页隐藏凭据值，
但用户命令和结果本身可能含敏感信息；控制台属于高权限界面。
...
需要更大的浏览器下载文件时，准备独立 HTTPS 入口并添加：
...
`public_url` 不会自动成为下载地址，OpenAI 私有 Tunnel 也不是浏览器文件下载入口。
...
高水位是 Code Mode 执行组件的近似内存压力阈值，不是整个机器或子进程树的硬上限。
...
这些是本服务的保守预算，不是通用 ChatGPT token 限制。补充正文上限 30,000 字节，问题答复连同题目和选择计费；
队首放不下就继续等待，不为它再次裁剪普通结果。消息和问题保留 72 小时、使用有界内存，容量不足时拒绝新提交。

会话沟通与执行存储独立：清空审计或 Web 重启执行服务保留补充、问题和备注；整个进程退出后不恢复。
