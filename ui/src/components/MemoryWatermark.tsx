...
export function MemoryWatermark({ memory }: MemoryWatermarkProps) {
  ...
  const getStatusBadge = () => {
    switch (memory.status) {
      ...
      case "exceeded":
        return (
          <span className="inline-flex max-w-full items-center gap-1.5 px-2 py-0.5 rounded text-[11px] font-medium bg-rose-50 dark:bg-rose-950/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800">
  ...
  return (
    <div className="mb-4 p-3.5 rounded-xl border border-zinc-200/80 dark:border-zinc-800/80 bg-white dark:bg-zinc-900/50 shadow-xs flex flex-col lg:flex-row lg:flex-wrap lg:items-center justify-between gap-3 text-xs">
      <div className="min-w-0 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-3">
      ...
      <div className="min-w-0 text-[11px] text-zinc-500 dark:text-zinc-400 flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-1 sm:gap-3 font-sans border-t sm:border-t-0 pt-2 sm:pt-0 border-zinc-100 dark:border-zinc-800">
